...
conda create -n dronecv python=3.8 && conda activate dronecv && pip install -r requirements.txt  # Conda

# Verify setup
python -m src.test_setup

# Run tests
python -m src.test_detection  # Detection test
python -m src.test_tracking   # Tracking test
```

### What to Expect
1. After running tests, check `output/` directory for:
   - Detection results: `detection_test_[timestamp]/`
   - Tracking results: `tracker_test_[timestamp]/`
   - Debug images showing detection/tracking process

### Customizing Tests
Edit these variables in test files to match your video:

```python
# In src/test_detection.py or src/test_tracking.py
VIDEO_PATH = "path/to/your/video.avi"  # Your video file
SCOPE_CENTER = (450, 340)  # Center of tracking area (x, y)
SCOPE_SIZE = (100, 100)    # Size of tracking window
...
1. Directory Structure
   ```
   output/
   ├── detection_test_[timestamp]/
   │   ├── debug_*.png           # Visualization frames
   │   ├── debug_log.json        # Detection metrics
   │   └── result_frame_*.jpg    # Processed frames
   └── tracker_test_[timestamp]/
       ├── initial_frame_*.jpg   # Starting points
       └── result_frame_*.jpg    # Tracking results
   ```

2. Debug Images
   - `*_diff.png`: Frame differences
   - `*_mask.png`: Detection masks
   - `*_threshold.png`: Binary thresholded images
   - `*_flow.png`: Optical flow visualization


For detailed configuration options, see Usage section below.
